package com.jcwang.store.seckill.service.impl;
...
import com.jcwang.store.seckill.feign.CouponFeignService;
import com.jcwang.store.seckill.feign.ProductFeignService;
import com.jcwang.store.seckill.interceptor.LoginUserInterceptor;
import com.jcwang.store.seckill.service.SeckillService;
import com.jcwang.store.seckill.to.SeckillSkuRedisTo;
import com.jcwang.store.seckill.vo.SeckillSessionWithSkusVo;
import com.jcwang.store.seckill.vo.SkuInfoVo;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    public void uploadSeckillSkuLatest3Days() {
        ...
        R lates3DaySession = couponFeignService.getLatest3DaySession();
    ...
    private void saveSessionInfos(List<SeckillSessionWithSkusVo> sessions) {
        ...
        sessions.stream().forEach(session -> {
            ...
            // 判断是否有，有的话删掉重新添加，因为如果给这个活动在重新多添加一个商品，他是加不上去的
            // 缓存活动信息
            Boolean hasKey = redisTemplate.hasKey(key);
            if (hasKey) {
                redisTemplate.delete(key);
            }
            if (session.getRelationSkus().size() != 0) {
                // 获取到活动中所有商品的skuId
                ...
                if (currentTime >= startTime && currentTime <= endTime) {
                    ...
                    if (listValue != null && listValue.size() >= 0) {
                        ...
                        List<SeckillSkuRedisTo> collect = listValue.stream().map(item -> {
                            ...
                            String seckillCount = redisTemplate.opsForValue().get(SKU_STOCK_SEMAPHORE + item.getRandomCode());
            ...
            if (randomCode.equals(key) && killId.equals(skuId)) {
                ...
                if (count > 0 && num <= seckillLimit && count >= num ) {
                    ...
                    if (aBoolean) {
                        ...
                        //保证Redis中还有商品库存
                        if (semaphoreCount) {
                            // 同时给redis也放一下缓存个数，这个在分布式下肯定有问题的，但是现在先这样了
                            // 而redissonClient是分布式的所以没问题。
                            redisTemplate.opsForValue().set(SKU_STOCK_SEMAPHORE + randomCode, (count-1) + "");
                            //创建订单号和订单信息发送给MQ
                    ...
                    log.error("此人已经购买过，请删除redis的setnx的占位然后再重新测试");
                }
                log.error("库存有问题");
            }
        }
